...
for col in float_columns:
    data[col]=data[col].rolling(window=75,center=False,min_periods=1).mean() #300 for KKK13 70 for 14 15 16 , 50for 17
...
#    x_testtest1[col] = ss.transform(x_testtest1[[col]]).squeeze()
    data[col]=ss.transform(data[[col]]).squeeze()
...
transfer_history=transfer_model.fit(x_train, x_train, epochs=50, batch_size=72, validation_split=0.1,callbacks= transfer_es)
...
#ax2.plot(time[indx_org],ldv2_acc_org.iloc[indx_org],c='0.45',linewidth=0.1,marker="o",markersize=3,markerfacecolor='none')
timearray=np.append(np.arange(0, time[-1], step=1.0),round(time[-1],1))
plt.xlabel("Time(s)", **hfont,fontsize=16)
plt.ylabel("Velocity(m/s)", **hfont,fontsize=16)
# plt.xlim(xmin=min(time), xmax=max(time))
plt.xticks(timearray,fontsize=12) 
